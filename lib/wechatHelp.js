...
class WechatHelp {
    ...
    async #getWechatFilePath() {
        ...
        let wechatFilePath = window.dbDevice("wechatFilePath");
        ...
        // 保存 微信文档目录路径 到数据库中去
        if (fs.existsSync(wechatFilePath)){
            window.dbDevice("wechatFilePath",wechatFilePath);
        }
    ...
    async #getBinPath() {
        ...
        let exePath = window.dbDevice("multiple_wechat");

        if (!exePath || !fs.existsSync(exePath)) {
            throw("未下载多开程序，请执行下载命令");
        }
    ...
    async startWx(wxid = 0) {
        ...
        if (wxid === 0 && window.dbDevice("show_tip") !== "1"){  // 打开一个新的微信
            window.dbDevice("show_tip","1");
    ...
    async saveWxData(){
        ...
        // 记录本次登陆的微信账号信息
        window.dbDevice("wx_" + wxData.id,JSON.stringify(wxData));

        return wxData;
    }

    async downloadWechatMuliteExe(){
        ...
        if (status !== 200) {
            ...
            throw("多开程序下载失败，请手动下载，放置目录：" + exePath);
        }
        ...
        window.dbDevice("multiple_wechat",exePath);
    }
}
