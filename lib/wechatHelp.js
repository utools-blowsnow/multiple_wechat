...
class WechatHelp {
    ...
    async #getWechatFilePath() {
        let wechatFilePath = this.wechatFilePath;
        if (fs.existsSync(wechatFilePath)){
            return wechatFilePath
        }
        // 1. 尝试从数据库中获取记录的微信文档目录路径
        wechatFilePath = window.dbDevice.getItem("wechatFilePath");
        ...
        this.wechatFilePath = wechatFilePath;

        return wechatFilePath;
    }

    saveWechatFilePath(path){

        // TODO 校验微信文档目录是否有问题

        this.wechatFilePath = path;

        window.dbDevice.setItem("wechatFilePath", path);
    }
    ...
    async getLocalWechatAccountList() {
        let wechatFilePath = await this.#getWechatFilePath();
    ...
    async startWx(itemData=null) {
        ...
        // 监听微信登陆完成
        // if (fs.existsSync(configPath)){
        //     let configPathStats = fs.statSync(configPath);
        //     // 新开个定时任务
        //     logger.log("ready 定时任务监听登陆完成", configPathStats)
        //     return new Promise((resolve, reject) => {
        //         let intervalTimer = setInterval(() => {
        //             let nowConfigPathStats = fs.statSync(configPath);
        //             if (configPathStats.mtimeMs !== nowConfigPathStats.mtimeMs){
